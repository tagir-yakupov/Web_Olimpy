...
ОГЭ Подготовка - Информатика и Математика
"""
import os
import sys
import random
...
from backend.admin_panel_th import conf_th  # ← Панель администратора
...
app.register_blueprint(conf_th)  # ← Регистрируем панель администратора

# Загружаем задания (кешируем при старте)
...
def _get_available_topics(subject: str) -> list:
    """Возвращает список тем для предмета"""
...
@app.route('/task')
def task_info():
    """Задания по информатике с фильтрацией"""
    if 'user_id' not in session:
        flash('Пожалуйста, войдите в систему', 'warning')
        return redirect(url_for('auth.login'))
...
@app.route('/task/math')
def task_math():
    """Задания по математике с фильтрацией"""
    if 'user_id' not in session:
        flash('Пожалуйста, войдите в систему', 'warning')
        return redirect(url_for('auth.login'))
...
def generate_variant_page():
    """Страница настройки генерации варианта"""
    if 'user_id' not in session:
        flash('Пожалуйста, войдите в систему', 'warning')
        return redirect(url_for('auth.login'))
...
def create_variant():
    """Создание варианта по параметрам"""
    if 'user_id' not in session:
...
def take_variant(subject: str):
    """Страница прохождения сгенерированного варианта"""
    if 'user_id' not in session:
        flash('Пожалуйста, войдите в систему', 'warning')
        return redirect(url_for('auth.login'))
